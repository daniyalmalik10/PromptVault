...
class TestGroqProvider:
    ...
    async def test_complete_success_returns_result(self, provider):
        ctx, _ = _mock_client(_make_response(200, COMPLETION_RESPONSE))
        with patch("apps.providers.groq.httpx.AsyncClient", return_value=ctx):
            result = await provider.complete("Say hello", "llama3-8b-8192")
    ...
    async def test_complete_sends_correct_body_and_auth_header(self, provider):
        ctx, client = _mock_client(_make_response(200, COMPLETION_RESPONSE))
        with patch("apps.providers.groq.httpx.AsyncClient", return_value=ctx):
            await provider.complete("My prompt", "llama3-8b-8192", max_tokens=512)
    ...
    async def test_complete_non_200_raises_provider_error(self, provider):
        ctx, _ = _mock_client(_make_response(401, {"error": "invalid_api_key"}))
        with patch("apps.providers.groq.httpx.AsyncClient", return_value=ctx):
            with pytest.raises(ProviderError) as exc_info:
                await provider.complete("prompt", "model")

        assert exc_info.value.status_code == 401

    async def test_complete_429_raises_provider_error_with_status(self, provider):
...
class TestOpenRouterProvider:
    ...
    async def test_complete_success(self, provider):
        ctx, _ = _mock_client(_make_response(200, COMPLETION_RESPONSE))
        with patch("apps.providers.openrouter.httpx.AsyncClient", return_value=ctx):
            result = await provider.complete("prompt", "openai/gpt-4o-mini")

        assert result.text == "Hello world"
        assert result.input_tokens == 10
        assert result.output_tokens == 20

    async def test_complete_sends_http_referer_header(self, provider):
        ctx, client = _mock_client(_make_response(200, COMPLETION_RESPONSE))
        with patch("apps.providers.openrouter.httpx.AsyncClient", return_value=ctx):
            await provider.complete("prompt", "model")
    ...
    async def test_complete_sends_correct_auth_header(self, provider):
        ctx, client = _mock_client(_make_response(200, COMPLETION_RESPONSE))
        with patch("apps.providers.openrouter.httpx.AsyncClient", return_value=ctx):
            await provider.complete("prompt", "model")
